...
└──────────┬───────────┘           └────────────┬────────────┘
           └──────────────────┬────────────────┘
                              ▼
┌─ Phase 2 ────────────────────────────────────────────────┐
│ GLM 5.3 逐文件编写代码（依赖分组并行，UI 文件由 Qwen）    │
└──────────────────────────┬───────────────────────────────┘
                           ▼
        ┌──────────────────┴──────────────────┐
        ▼                                     ▼
...
每个文件只注入三样：①该文件的规格；②`依赖:` 声明的文件内容（各截 4000 字符）；③输出约束（只输出完整代码，围栏残留被 `strip_fences` 剥掉）。这是原始架构「Do NOT give the coder the entire spec at once」原则的实现（v1.4 起 coder 为 GLM 5.3）。
